//
//  Rob8Chat.swift
//  Polly  —  portfolio excerpt
//
//  On-device conversational AI chat built on Apple's FoundationModels
//  framework (Apple Intelligence). Runs 100% offline: no network, no API key.
//
//  This file is an excerpt: ChatView references a `GameManager`
//  EnvironmentObject from the full app, intentionally omitted here.
//
...
// MARK: - Chat View Model (FoundationModels integration)
...
class ChatViewModel: ObservableObject {
    ...
    // The on-device language model session. It keeps conversation
    // context across turns automatically.
    private let session: LanguageModelSession

    init() {
        // System prompt: define Rob8's persona, tone and scope.
    ...
    func send(userText: String, onFunIncrease: () -> Void) async {
        ...
        do {
            // One call → the model answers, fully on-device.
...
struct ChatView: View {
    @EnvironmentObject var game: GameManager      // from the full app (omitted in this excerpt)
